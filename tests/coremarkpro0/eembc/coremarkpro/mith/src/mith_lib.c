...
int mith_report_results(ee_workload *workload, size_t total_time, e_u32 num_contexts, unsigned int num_workers) {
	...
	mith_log_u(wname,"time(ticks)",total_time);
	for (i=0; i<workload->max_idx ; i++)
	{
		ee_work_item_t *item=workload->load[i];
	...
	/* only output per item information if verification is turned on (except nbitems/s) */
	if (verify_output) {
	...
	}
	{
		for (i=0; i<workload->max_idx ; i++)
		{
			...
			if (verify_output) mith_log(name,"UID",item->uid);
			if (verify_output) mith_log(name,"fails",item->failed);
			if (verify_output) mith_log(name,"time(ticks)",item->time);
			total_item_time+=item->time;
			if (verify_output) mith_log(name,"count",item->finished);
		if (verify_output) 	mith_log_tcdef(name,item->tcdef,item->time,item->finished);
	#if FLOAT_SUPPORT
			secs=(double)item->time/(double)al_ticks_per_sec();
			total_item_secs+=secs;
			if (verify_output) mith_log_dbl(name,"time(secs)",secs);
			if (verify_output) mith_log_dbl(name,"secs/item",secs/(double)item->finished);
			mith_log_dbl(name,"items/sec",(double)item->finished/secs);
      sprintf(report_file+strlen(report_file),"%s;%f\n",name,(double)item->finished/secs);
	#endif
		}
		if (verify_output) mith_log("Items","total(ticks)",total_item_time);
	#if FLOAT_SUPPORT
		...
		if (verify_output) mith_log_dbl("Items","total(secs)",total_item_secs);
		#if OVERHEAD_REPORT
